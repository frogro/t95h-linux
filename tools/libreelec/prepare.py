...
def freetype_archive(text):
 # Avoid Savannah's failing redirector; keep the original archive/hash.
 return text.replace('https://download.savannah.gnu.org/releases/freetype/', 'https://download-mirror.savannah.gnu.org/releases/freetype/')
...
def thermal_policy(dtb):
 ...
 def read(node,prop):
  return int(subprocess.check_output(['fdtget','-t','u',str(dtb),trips+node,prop],text=True).strip())
...
'''
 ...
 # Keep LibreELEC's standard Bluetooth stack and adapter support even on
 # boards without an onboard controller. Do not import unrelated hardware.
 upstream_cfg=config((le/'projects/Allwinner/linux/linux.aarch64.conf').read_text())
...
''';pkg.write_text(s.replace(marker,marker+case))
 ...
 report={'upstream':lock,'kernel':bl['kernel'],'kernel_patches':[sha(x[0]) for x in patches],'profile':'base-B','thermal_policy':thermal,'gpu_runtime_pm':'on','cpu_governor':'ondemand','firmware':selected,'hardware_tested':False,'bootloader_rebuilt':False,'image_ready':False,'required_kernel_config':required+bluetooth_required,'excluded_allwinner_kernel_patches':sorted(p.name for p in (le/'projects/Allwinner/patches/linux').glob('*.patch'))}
