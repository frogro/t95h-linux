			 ...
			 struct ieee80211_vif *vif)
{
	...
	/* drv_priv embeds xradio_vif; it is not a pointer slot.
	 * Initialize the lock before publishing vif_list to the BH. */
...
#if defined(XRADIO_MAC80211_HAS_RADIO_IDX) || (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 17, 0))
		  int radio_idx,
#endif
		  u32 changed)
{
...
static void xradio_pending_offchanneltx_work(struct work_struct *work)
{
	struct xradio_vif *priv =
...
int xradio_vif_setup(struct xradio_vif *priv)
{
	...
	/* vif_lock is initialized before vif_list publication. */
