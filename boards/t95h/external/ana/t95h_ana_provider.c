...
static int sun50i_h6_ppu_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < data->nr_domains; i++) {
		...
		if (data == &sun50i_h616_ppu_data && desc->mask == PD_H616_ANA_VDD_GATE) {
            struct arm_smccc_res r;
            arm_smccc_smc(0xc000ff95, 0, 0, 0, 0, 0, 0, 0, &r);
            if (r.a0 != 0x54413935 || r.a3 != 12 || !(r.a1 & BIT(4))) {
                ret = -EINVAL;
                dev_err(dev, "ANA test refused: EL3 does not confirm enabled ANA\n");
                goto out_remove_pds;
            }
            dev_info(dev, "ANA test: EL3=%lx; initializing ANA as on, no ANA register write\n", r.a1);
            ret = pm_genpd_init(&pd->genpd, NULL, false);
        } else {
            ret = pm_genpd_init(&pd->genpd, NULL, !sun50i_h6_ppu_power_status(pd));
        }
		if (ret) {
